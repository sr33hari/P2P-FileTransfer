...
class ChatClient(Frame):
  ...
  def initUI(self):
    self.root.title("Welcome to the p2p chat and share window")
    ...
    clientShareButton = Button(ipGroup, text="Start", width=10, command=self.shareClient)
    ...
    clientShareButton.grid(row=0, column=9, padx=5)
    ...
    bottomLabel = Label(parentFrame, text="P2P chat requests for file sharing")
    bottomLabel = Label(parentFrame, text="Click the start button if you want to start sharing files")
  ...
  def handleAddClient(self):
	...
	#call(["python", "filergui.py", "4555", "3", "192.168.0.106:4555"])

  def shareClient(self):
    call(["python", "tk.py"])
